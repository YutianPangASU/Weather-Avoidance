...
class test_weather_lstm(object):
    ...
    def conv_lstm_graph_2(self, x, x_conv, y_true):
        ...
        #h_t_0 = tf.zeros([batch_size, dim_out], name='h_t_0')
        c_t_0 = y_true[:, 0, :]
        h_t_0 = tf.layers.dense(y_true[:, 0, :], dim_hid-2, activation=tf.nn.relu, name='fc_in', reuse=tf.AUTO_REUSE)
        ...
        for t in range(time_steps-1):
            ...
            h_t_new = tf.layers.dense(h_t_0, dim_hid - 2, activation=tf.nn.relu, name='fc_mid',
                                      reuse=tf.AUTO_REUSE)
            # update parameters
            h_t_0 = h_t_new
            c_t_0 = c_t
