...
def clear_all():
    ...
    # remove start_end_corr.csv
    if os.path.exists("start_end_corr.csv"):
        os.remove("start_end_corr.csv")
    else:
        print("The file start_end_corr.csv.csv does not exist")

    # remove start_end_uncorr.csv.csv
    if os.path.exists("start_end_uncorr.csv"):
        os.remove("start_end_uncorr.csv")
    else:
        print("The file start_end_uncorr.csv does not exist")
    ...
    for y_train, start_end in itertools.izip(csv_f1, csv_f2):
        ...
        with open('values_diag.csv', 'a') as f:
            ...
            if max >= threshold:
                ...
                with open('start_end_uncorr.csv', 'a') as f4:
                    f4writer = csv.writer(f4, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
                    f4writer.writerow(start_end)
            c += 1
